...
function KpiCard({ title, value, icon, trend, delay }) {
  return (
    <div className={`fade-up ${delay} bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md transition-shadow duration-200`}>
      ...
      {trend !== undefined && <p className="text-xs text-gray-400 mt-1">{trend}</p>}
    </div>
  );
}
          ...
          <div className="divide-y divide-gray-100">
            {Array(5).fill(0).map((_, i) => (
              <div key={i} className="flex items-center gap-4 px-5 py-3.5 animate-pulse">
                ...
                <div className="flex-1 space-y-1.5"><div className="h-3 bg-gray-100 rounded w-1/3" /><div className="h-2.5 bg-gray-100 rounded w-1/5" /></div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50">
                  {["Document","Status","Routing","Department","Date",""].map(h => (
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map(doc => (
                  <tr key={doc.id} onClick={() => navigate(`/documents/${doc.id}`)}
                    ...
                    <td className="px-4 py-3"><span className="text-sm text-gray-600 capitalize">{doc.department || "—"}</span></td>
